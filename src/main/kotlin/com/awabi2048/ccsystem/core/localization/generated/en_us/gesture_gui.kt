...
internal object EnUsGestureGuiCatalog {
    ...
    fun entries(): List<EmbeddedLocalizationEntry> = listOf(
        ...
        entry("gesture_gui.debug.subtitle", "§7Gaze:{inside} Envelope:{envelope} hit:{hit} Dist:{dist}/{range} {motion}{dirty}{dummy} Delta:{displacement}"),
        entry("gesture_gui.debug.resummon", "§e[Gaze] Resummoned screens (delta:{displacement}m, gaze outside)"),
        entry("gesture_gui.debug.resummon_failed", "§c[Gaze] Failed to resummon: {detail}"),
        entry("gesture_gui.debug.dummy_started", "§e[Gaze] Started dummy follow (delta:{displacement}m, gaze outside)"),
