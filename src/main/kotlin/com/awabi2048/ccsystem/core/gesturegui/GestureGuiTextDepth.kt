 ...
 * 画面中心基準で単純に縮めると、layer 6の文字がlayer 4のボタンの背面へ
 * 回り込むため、標準コンテンツ面からの距離を4分の1にする補間として定義します。
 * これにより、layer 4のボタン上のlayer 6の文字はlayer 4.5へ、既定layer 20の
 * 文字はlayer 8へ移動し、既存の面より前面という関係を保ちます。
 */
internal object GestureGuiTextDepth {
    /** 標準的なボタン面を表すBlockDisplayのlayerです。 */
    const val CONTENT_SURFACE_LAYER: Double = 4.0

    /** コンテンツ面からTextDisplayまでの距離を残す割合です（元の25%）。 */
    const val SURFACE_DISTANCE_RATIO: Double = 0.25
